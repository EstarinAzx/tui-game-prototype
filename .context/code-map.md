...
| File | What lives here |
|---|---|
| [core.py](../karma_rush/core.py) | `GameState` — all game rules; holds a `Maze`. [`tick`](../karma_rush/core.py#L148) advances one Tick: decay → move (Wall-blocked) → pickup (+ Bonus-time roll) → end-check. [`_refill_items`](../karma_rush/core.py#L115) restocks Floor-only to `item_cap`; [`_roll_karma`](../karma_rush/core.py#L95) flips the 50/50 Karma, [`_roll_bonus`](../karma_rush/core.py#L105) rolls Bonus time on a good Pickup. `Pickup` event record at L26 (carries `bonus_seconds`). |
...
| [config.py](../karma_rush/config.py) | `Config` frozen dataclass — every tuning constant. `DEFAULT` preset at L84. The only place to rebalance. |
...
| Karma | [`Pickup.karma`](../karma_rush/core.py#L31), `karma_good`/`karma_bad`, [`_roll_karma`](../karma_rush/core.py#L95) |
| Bonus time | [`Pickup.bonus_seconds`](../karma_rush/core.py#L33), `GameState.bonus_time_total`, `bonus_time_chance`/`bonus_time_amount`, [`_roll_bonus`](../karma_rush/core.py#L105); `_run_length` extends the clock |
| Item | `{cell: karma}` entries in `GameState.items` |
| Pickup | [`Pickup`](../karma_rush/core.py#L26) record, returned by `tick` |
...
| Question | Start at |
|---|---|
| "Why did sanity change?" | [core.py:156](../karma_rush/core.py#L156) — passive decay, then karma on pickup |
| "Why did the run end?" | [core.py:187](../karma_rush/core.py#L187) — sanity-first end-check; `"time"` measures `_run_length` |
| "Where is a Pickup resolved?" | [core.py:172](../karma_rush/core.py#L172) |
| "How is Bonus time granted?" | [core.py:179](../karma_rush/core.py#L179) — good-Karma `_roll_bonus`, banked in `bonus_time_total` |
| "How does an Item spawn?" | [core.py:115](../karma_rush/core.py#L115) — `_refill_items`, Floor cells only |
| "Why is a finished run frozen?" | [core.py:151](../karma_rush/core.py#L151) — `tick` no-ops once `run_over` |
...
| "Why can't the player move?" | [core.py:167](../karma_rush/core.py#L167) — per-axis `maze.is_floor` check |
